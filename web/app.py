...
def linkable_converted_date(date):
  ...
  today = carpe_diem(specific_date, engine)
...
def create_webhook():
  ...
  from repcal_shared import kubernetes
  ...
  try:
    api_response = kubernetes.create_cronjob(url=url, time_zone=timezone, schedule=schedule)
...
def success():
  ...
  from webhook.webhook import construct_embed, use_webhook
  time = datetime.now()
  today = carpe_diem(time, engine)
  data = {
    "day": today.day,
